...
async def login_for_access_token(
    ...
    """Create an access token for the user (if they successfully log in)."""
    # Look up user by email
    # Note: OAuth2PasswordRequestForm uses "username" field, but we treat it as the email
    result = await db.execute(
        select(models.User).where(
...
async def forgot_password(
    ...
    """Generate a password reset token and send a reset email if the user exists."""
    result = await db.execute(
        select(models.User).where(
...
async def reset_password(
    ...
    """Reset a user's password if provided with a valid reset token."""
...
async def get_user(
    ...
    """Get the current user (public values)."""
...
async def upload_profile_picture(
    ...
    if len(content) > settings.max_upload_size_profile_pic:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"File too large. Maximum size is {settings.max_upload_size_profile_pic // (1024 * 1024)}MB",
        )
